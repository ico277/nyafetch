...
static inline uint32_t crc32(const void* d, size_t l) {
...
static inline uint64_t crc64(const void* d, size_t l) {
    const uint8_t* d2 = (const uint8_t*)d;
    uint64_t crc = 0;
    for (size_t i = 0; i < l; ++i) {
...
static inline uint32_t strcrc32(const char* s) {
    uint32_t crc = 0;
    char c;
    while ((c = *s)) {
...
static inline uint64_t strcrc64(const char* s) {
    uint64_t crc = 0;
    char c;
    while ((c = *s)) {
...
static inline uint32_t strcasecrc32(const char* s) {
    uint32_t crc = 0;
    char c;
    while ((c = tolower(*s))) {
        crc = crc32_table[(crc ^ c) & 0xFF] ^ (crc >> 8);
...
static inline uint64_t strcasecrc64(const char* s) {
    uint64_t crc = 0;
    char c;
    while ((c = tolower(*s))) {
        crc = crc64_table[(crc ^ c) & 0xFF] ^ (crc >> 8);
